...
document.addEventListener('DOMContentLoaded', function() {
    ...
    navItems.forEach(item => {
        ...
        // Match exact path or parent path (more specific matching than before)
        if (currentLocation === href || 
            (href !== '/' && currentLocation.startsWith(href) && 
            (currentLocation.length === href.length || currentLocation[href.length] === '/')) {
...
function setupResumeCardInteractions() {
    ...
    resumeCards.forEach(card => {
        card.addEventListener('click', function(event) {
            ...
            const allDropdowns = document.querySelectorAll('.resume-actions-dropdown');
            allDropdowns.forEach(d => {
                if (d.id !== `actions-dropdown-${resumeId}`) {
